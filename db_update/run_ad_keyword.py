...
def main():
    ...
    if TARGET_MODE == "overlap":
        for ads in batches:
            _process_batch(
                conn, extractor, ads, now_kst, DEBUG,
                TARGET_MODE,
                total_processed, total_failed, total_fixed, total_still_bad
            )
    else:
        last_ad_id = 0
        batch_num  = 0
        ...
        while True:
            ...
            processed, failed, fixed, still_bad = _process_batch(
                conn, extractor, ads, now_kst, DEBUG, TARGET_MODE
            )
...
def _process_batch(conn, extractor, ads, now_kst, debug, target_mode):
